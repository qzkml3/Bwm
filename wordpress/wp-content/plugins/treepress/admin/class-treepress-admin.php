...
<?php 
        ...
        if ( isset( $_POST['submit'] ) ) {
            ...
            foreach ( $gedcom->getIndi() as $individual ) {
                ...
                $individuals['persons'][$individual->getId()]['name'] = current( $individual->getName() )->getName();
    ...
    public function account_settings()
    {
        ...
        ?>   https://www.bookpress.net/docs/</p>
		<p><strong> <?php 
    ...
    public function options_panel()
    {
        ...
        if ( isset( $_POST['update_options'] ) ) {
            ...
            if ( $active_tab == 'to' ) {
                update_option( 'show_biodata_on_posts_page', $this->validate_option( 'show_biodata_on_posts_page' ) );
        ...
        ?>  </a>
			   <?php 
        ?>
				<a href="?page=treepress-options&tab=cr" class="nav-tab <?php 
        echo  ( $active_tab == 'cr' ? 'nav-tab-active' : '' ) ;
        ?>"><?php 
        _e( 'Credit', 'treepress' );
        ?>  </a>
			</h2>
...
<br>
				<h3><?php 
            _e( 'General settings', 'treepress' );
            ?></h3>
				<table class="form-table">
					<tr valign="top">
						<th scope="row"><label for="show_biodata_on_posts_page"><?php 
            _e( 'Show biodata info on single', 'treepress' );
            ?></label></th>
						<td><input name="show_biodata_on_posts_page" type="checkbox" id="show_biodata_on_posts_page" value="Y" <?php 
            echo  ( $treepress->options->get_option( 'show_biodata_on_posts_page' ) == 'true' ? ' checked' : '' ) ;
            ?> /></td>
					</tr>
				</table>
...
<?php 
    ...
    public function render_meta_box_member()
    {
        ...
        ?>
				...
				<tr>
					<td></td>
					<td><button type="button" class="add_more_fact"  style="position:relative;left:-3px;">+</button></td>
				</tr>
				</table>
				<?php 
    }
    ...
    public function save_post_member( $id )
    {
        ...
        if ( isset( $_POST['more_fact_value'] ) ) {
            ...
            update_post_meta( $id, 'more_fact_value', sanitize_text_field( $_POST['more_fact_value'] ) );
        }
